...
public class TrackAprilTagSubsystem extends SubsystemBase {
    ...
    // *******************************************************************
    // Camera variables
    private String name_leftcamera, name_rightcamera;  // Holds the names of the cameras as defined in the Hub's configuration file.
    ...
    public TrackAprilTagSubsystem(HardwareMap hardwareMap) {

        // Create the AprilTag processor by using a builder.
        aprilTagProcessor = new AprilTagProcessor.Builder().build();
        if (aprilTagProcessor == null) {
            ableToRunVision = false;
            telemetry.addData("VISION", "Unable to build the aprilTagProcessor for vision.");
            return;
        }
        ...
        // Let's default to the left side camer
    ...
    public void periodic() {
        ...
        if (!ableToRunVision || aprilTagProcessor == null || desiredTagID <= 0 || visionPortal == null) {
            telemetry.addData("VISION", "Unable to run vision for AprilTags.");
            return;
        }
        ...
        if (currentDetections == null) {
            telemetry.addData("VISION", "Unable to find any AprilTags.");
            return;
        }
        ...
        for (AprilTagDetection detection : currentDetections) {
            ...
            if (detection.metadata != null) {
                ...
                if ((desiredTagID > 0) || (detection.id == desiredTagID)) {
                    ...
                    // Tag is found!!!
                    telemetry.addData("Found", "Tag ID %d found.", detection.id);
    ...
    */
    private boolean setManualExposure(int exposureMS, int gain) {
        ...
        if (!ableToRunVision || visionPortal == null) {
            telemetry.addData("VISION", "Unable to set manual exposure on cameras.");
            return false;
        }

        // Make sure camera is streaming before we try to set the exposure controls
        if (visionPortal.getCameraState() != VisionPortal.CameraState.STREAMING) {
            telemetry.addData("VISION", "Unable to set manual exposure on cameras. Not streaming.");
            telemetry.update();
            return false;
        }
        ...
        telemetry.addData("VISION", "Set manual exposure on cameras.");
        return true;
    }
